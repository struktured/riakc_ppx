...
module Make_with_usermeta_index_primitive_key
  ...
  module Key = Core.Std.String (*Key*)
  module Value = Value
  module Usermeta_value = Usermeta_value
  module Index_value = Index_value
			 
  let serialize_key = fun x -> x  (*serialize_proto Key.to_protobuf*)
  let serialize_value = serialize_proto Value.to_protobuf 

  let deserialize_key = fun x -> x (* deserialize_proto Key.from_protobuf  *)
  let deserialize_value = deserialize_proto Value.from_protobuf
