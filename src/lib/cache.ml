...
module Make_with_usermeta_index_primitive_key
  ...
  let deserialize_key = Key.to_string
  let deserialize_value = deserialize_proto Value.from_protobuf
  ...
  let list_keys cache = Conn.list_keys cache.conn cache.bucket >>| function
								 | Result.Ok keys ->
								    Result.Ok (List.map (fun (b:string) -> b (*Key.from_protobuf (Protobuf.Decoder.of_string (encode_decode b)) *) ) keys)
								 | Result.Error err ->
								    Result.Error err
										 
  let get cache ?(opts = []) (k:Key.t) = Conn.get cache.conn ~opts ~b:cache.bucket k 
					 >>| function
					   | Result.Ok robj_unsafe -> begin
								      let robj = Robj.from_unsafe robj_unsafe in
								      if Robj.contents robj = [] && Robj.vclock robj = None then
  ...
  let put cache ?(opts = []) ?(k:Key.t option) (robj:'a Robj.t) =
    let unsafe_robj = Robj.to_unsafe robj in
    let serialized_key = Option.map k (*serialize_key*) (fun x -> x) in
    Conn.put
      cache.conn
      ~opts
      ~b:cache.bucket
...
module Make_with_primitive_keys(Value:Protobuf_capable.S) =
  Make_with_usermeta_index_primitive_key(Value) (Default_usermeta) (Default_index)
